// Handles GET /api/votes and POST /api/votes
...
export async function POST(req: NextRequest) {
  ...
  const { property_id, vote_option_id } = await req.json();
  const user_id = (req as AuthNextRequest).user!.id;

  if (!property_id || !vote_option_id) {
    return NextResponse.json({ success: false, error: 'Please provide property_id and vote_option_id' }, { status: 400 });
  }
  ...
  try {
    ...
    if (existingVote.rows.length > 0) {
      return NextResponse.json({ success: false, error: 'You have already voted for this property' }, { status: 400 });
    }
