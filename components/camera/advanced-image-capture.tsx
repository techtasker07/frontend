...
export function AdvancedImageCapture({
    ...
    if (videoRef.current && canvasRef.current) {
      ...
      if (context) {
        ...
        try {
          ...
          stopCamera()
          console.log('📷 Photo captured, closing camera view instantly...')

          // Show property details form immediately with loading state
          setShowPropertyForm(true)

          // Process image in background
          processImageInstantly(file)
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.type.startsWith("image/")) {
      console.log('📁 File selected, showing form instantly...')

      // Show property details form immediately
      setShowPropertyForm(true)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white via-purple-50 to-pink-50 relative overflow-x-hidden">
      ...
      {/* Property Details Form - Shows immediately after capture */}
      {showPropertyForm && user && (
        <PropertyDetailsForm
          imageUrl={processedImageUrl}
          identifiedCategory={identifiedCategory || undefined}
          propertyDetails={propertyDetails || undefined}
          prospects={prospects.length > 0 ? prospects : undefined}
          userId={user.id}
